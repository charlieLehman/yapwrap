...
class AttentionBlock(nn.Module):
    def __init__(self, in_planes):
        ...
        self.conv1 = nn.Conv2d(in_planes, 64, kernel_size=1, stride=1, bias=False)
        self.bn1 = nn.BatchNorm2d(64)
        self.conv2 = nn.Conv2d(64, 1, kernel_size=1, stride=1, bias=False)
        self.bn2 = nn.BatchNorm2d(1)

        self.shortcut = nn.Sequential(
                nn.Conv2d(in_planes, 1, kernel_size=1, stride=1, bias=False),
                nn.BatchNorm2d(1)
            )
...
class ImpAttn(nn.Module):
    ...
    def pixelwise_classification(self, x):
        ...
        out = self.layer4(out)
        if self.training:
            _s = s if self.tiny else (out.size(2), out.size(3))
        else:
            _s = s
        ...
        if self.training:
            attn = torch.sigmoid(torch.stack([self.upsample(x,_s) for x in [attn0]]).sum(0))
        else:
            attn = _impattn
        out = px_log*attn
    ...
    def get_attn_params(self):
        # modules = [self.attn0, self.attn1, self.attn2, self.attn3, self.attn4]
        modules = [self.attn0, self.attn1]
        for i in range(len(modules)):
            for m in modules[i].named_modules():
                if isinstance(m[1], nn.Conv2d) or isinstance(m[1], nn.BatchNorm2d):
                    for p in m[1].parameters():
...
# class ImpAttn(nn.Module):
#     def __init__(self, block, num_blocks, num_classes=10, tiny=True):
#         super(ImpAttn, self).__init__()
#         self.name = self.__class__.__name__
#         self.in_planes = 64
...
#         self.attn0 = AttentionBlock(64*block.expansion)
#         self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1)
#         self.attn1 = AttentionBlock(64*block.expansion)
#         self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=2)
#         self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2)
...
#         _impattn = self.impattn(px_log)
#         if self.training:
#             attn = torch.sigmoid(self.upsample(attn,s))
#         else:
#             attn = _impattn
#         out = px_log*attn
#         pred = out.sum((-2,-1))/attn.sum((-2,-1))
